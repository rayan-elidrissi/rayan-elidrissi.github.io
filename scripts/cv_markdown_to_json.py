...
def parse_craft(craft_dir):
    """Parse craft items from the _craft directory."""
    craft = []
    
    if not os.path.exists(craft_dir):
        return craft
    
    for craft_file in sorted(glob.glob(os.path.join(craft_dir, "*.md"))):
        with open(craft_file, 'r', encoding='utf-8') as file:
            content = file.read()
        ...
        if front_matter_match:
            ...
            # Extract craft details
            craft_entry = {
                "name": front_matter.get('title', ''),
                "category": front_matter.get('collection', 'craft'),
                "date": front_matter.get('date', ''),
                "url": front_matter.get('permalink', ''),
                "description": front_matter.get('excerpt', '')
            }
            
            craft.append(craft_entry)
...
def create_cv_json(md_file, config_file, repo_root, output_file):
    ...
    # Add craft
    cv_json["craft"] = parse_craft(os.path.join(repo_root, "_craft"))
